...
//NOTE: Since Orbital is just a copy of CompFunction, this file is useless now.
...
/** @brief Constructor for scalar Orbitals
 *
...
Orbital::Orbital(int spin, int occ, int rank)
    ...
    if (this->occ() < 0) {
        if (this->spin() == SPIN::Paired) this->data.n1 = 2; // TODO: changer ça pour que ça utilise les vecteurs n1 etc
        if (this->spin() == SPIN::Alpha) this->data.n1[0] = 1;
        if (this->spin() == SPIN::Beta) this->data.n1[1] = 1;
    }
}
...
// Copy of the original Orbital.h file for safekeeping. 
